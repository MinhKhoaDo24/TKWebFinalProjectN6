$(document).ready(function () {
    // ==========================================
    // 1. KHỞI TẠO BIẾN & LẤY THAM SỐ TỪ URL
    // ==========================================
    const params = new URLSearchParams(window.location.search);
    const genre = params.get('genre');
    const country = params.get('country');
    const type = params.get('type'); // Trạng thái: trending, now-showing, coming-soon
    ...
    // ==========================================
    // 2. TẢI DỮ LIỆU & LOGIC LỌC PHIM CHI TIẾT
    // ==========================================
    ...
    // ==========================================
    // 3. HÀM HIỂN THỊ DANH SÁCH PHIM (GRID)
    // ==========================================
    function renderPage(page) {
        ...
        if (moviesToShow.length === 0) {
            movieGrid.html('<div class="col-span-full text-center py-20 text-gray-500">Không tìm thấy phim nào phù hợp với yêu cầu.</div>');
                    ...
                    <a href="movie_detail.html?id=${m.id}" class="block">
                        <div class="aspect-[2/3] w-full rounded-2xl overflow-hidden bg-gray-800 mb-3 relative border border-white/5 shadow-lg">
                            ...
                            <div class="absolute inset-0 flex items-center justify-center opacity-0 group-hover/card:opacity-100 transition bg-black/40">
                                <div class="w-12 h-12 bg-purple-600 rounded-full flex items-center justify-center shadow-xl">
        ...
        movieGrid.html(html);
        renderPagination();
        window.scrollTo({ top: 0, behavior: 'smooth' }); // Cuộn lên đầu trang sau khi chuyển trang
    }

    // ==========================================
    // 4. HÀM PHÂN TRANG (PAGINATION)
    // ==========================================
